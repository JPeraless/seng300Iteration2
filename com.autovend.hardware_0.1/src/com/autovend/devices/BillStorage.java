...
/**
 * Represents devices that store bills. They only receive bills, not
 * dispense them. To access the bills inside, a human operator needs to
 * physically remove the bills, simulated with the {@link #unload()} method.
 * A {@link #load(Bill...)} method is provided for symmetry.
 */
public class BillStorage extends AbstractDevice<BillStorageObserver> implements Acceptor<Bill> {
	...
	/**
	 * Creates a bill storage unit that can hold the indicated number of
	 * bills.
	 * 
	 * @param capacity
	...
	/**
	 * Allows a set of bills to be loaded into the storage unit directly.
	 * Existing bills in the dispenser are not removed. Causes a
	 * "billsLoaded" event to be announced. Disabling has no effect on
	 * loading/unloading.
	 * 
	 * @param bills
	 *            A sequence of bills to be added. Each cannot be null.
	 * @throws SimulationException
	 *             if the number of bills to be loaded exceeds the capacity of
	 *             the unit.
	 * @throws SimulationException
	...
	/**
	 * Unloads bills from the storage unit directly. Causes a
	 * "billsUnloaded" event to be announced.
	 * 
	 * @return A list of the bills unloaded. May be empty. Will never be null.
	 */
	public List<Bill> unload() {
		List<Bill> bills = Arrays.asList(storage);

		storage = new Bill[storage.length];
		nextIndex = 0;
	...
	/**
	 * Causes the indicated bill to be added to the storage unit. If successful,
	 * a "billAdded" event is announced to its listeners. If a successful
	 * bill addition causes the unit to become full, a "billsFull" event is
